...
(defun prog-err (s)
  (if (/= s "quit / exit abort")
      (princ (strcat "\nError: " s)) ;; Mostrar el mensaje de error real
  )
  (setq _error_ olderr)
  (setq seleccion nil)
...
;; -------------------------------------------------------------------------------
;; FUNCION para crear las carpinter�as
;; -------------------------------------------------------------------------------
(defun carpint ()
  ;;Dibujar un perfil
  (setvar "CLAYER" "0")
...
(defun ventana (/ bisagra cosamuro datosmuro angulo
  ...
  (setq olderr *error* _error_ prog-err)
  ...
  (setq bisagra (getpoint "\nPunto en el que ir� una bisagra <Relativo>: "))
  (if (not bisagra)
    (setq bisagra (pto-rel))
  )
  (setq bisagra (osnap bisagra "_nearest")) ;;Ajustar el punto. Si da error se que no hab�a muro
  ...
  (if (not pgrosor)
    (progn
      (prompt "\nNo encuentro otra l�nea paralela")
      (exit) ;; Provocar fallo
    )
  )
  ...
  ;;hacia qu� lado?
  (initget 1)
  (setq aux (getpoint bisagra "\nHacia que lado de la bisagra ir� el hueco: "))
    ...
    (progn
      ...
      (if (= tipo "Doble")
        (progn
          ...
          (carpint)
          ;; Por �ltimo, la l�nea de en medio de la ventana
      ...
      ;; Esto lo inicialic� al principio    
      (setq hueco (+ perfil cristal perfil))
      (cond
      ...
      ;; Si quiero centrar la carpinter�a en el muro:
      (if (= centrar "Si")
        (progn
      ...
      ;; Si no lleva alfeizar, dibujar una l�nea
      (if (eq alf "No") 
  ...
  ;; dibujar la otra l�nea del cerco en el muro
  ...
  (recupera-vars)
  (prin1) ;Para que no salga ningun valor en la l�nea de comandos
)
